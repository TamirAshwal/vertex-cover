...
void findMinVertexCover(int graph[MAX_VERTICES][MAX_VERTICES], int n) {
    ...
    int minSize = n + 1;
    int lastPowerOf2 = 1;
    // find the first power of 2 that we get a valid answer this was by far my biggest improvement for the optimization
    ...
    for (int k = (lastPowerOf2 / 2) + 1; k <= lastPowerOf2 * 2 && minSize > k; k++) {
        generateCombinations(n, k, cover, 0, 0, graph, minCover, &minSize);
    }
